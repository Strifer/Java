...
/** 
 * This class represents a single for-loop construct. It inherits from Node. It contains methods for retrieval of attributes and a simple string conversion method used for simple output.
 * <p> A valid for-loop contains a single variable and two or three expressions which follow.
 * @author Filip Džidić
 *
 */
public class ForLoopNode extends Node {
	...
	/** Constructor which recieves references to user defined for-loop parameters. StepExpression can be null. */
	...
	/**
	 * This method returns a <code>String</code> representation of the user defined for-loop.
	 */
	public String toString() {
		String s;
		if (getStepExpression()==null) {
			s=("{$FOR "+getVariable().asText()+" "+getStartExpression().asText()+" "+getEndExpression().asText()+"$}");
		} else {
			s=("{$FOR "+getVariable().asText()+" "+getStartExpression().asText()+" "+getEndExpression().asText()+" "+getStepExpression().asText()+"$}");
		}
		s.concat("\n\r");
