...
public class ValueWrapper {
	...
	/**
	 * Performs the appropriate arithmetic operation as decided by the given operator.
	 * @param operator defines the operation being performed
	 * @param arg the second operand in our arithmetic operations
	 */
	private void performOperation(char operator, Object arg) {
		...
		switch (operator) {
		case '+':
			execute(operatingWithDoubles, (Number) value, (Number) arg,
					(x, y) -> (x + y));
			break;
		case '-':
			execute(operatingWithDoubles, (Number) value, (Number) arg,
	...
	/**
	 * Converts a valid <code>String</code> into a <code>Number</code> representation used for arithmetic operations.
	 * @param incValue the operand in <code>String</code> format
	...
	/**
	 * Executes our arithmetic operation and sets the stored value to the newly calculated value.
	 * @param operatingWithDoubles marks if the result will be a double or an integer
	...
	/**
	 * A functional interface which defines a single method used for performing arithmetic operations.
	 * @author Filip Džidić
	 *
	 */
	@FunctionalInterface
	private interface FOperation {
		/**
		 * Performs some kind of binary arithmetic operation on the two provided parameters
		 * @param x the first operand
		 * @param y the second operand
