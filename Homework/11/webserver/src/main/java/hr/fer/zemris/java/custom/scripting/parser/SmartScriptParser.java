...
/**
 * This class represents a script parser which parses through text trying to see
 * if it follows a pre defined format. The parsed document is stored as a tree
 * of <code>Node</code> elements where the root of the tree is a
 * <code>DocumentNode</code>.
 * <p>
 * Comes equipped with several methods for parsing the document, retrieving the
 * root of the constructed document tree and retrieving the text reconstruction
 * of said tree.
 * 
 * @author Filip Džidić
 * 
...
public class SmartScriptParser {
	...
	/**
	 * This constructor takes a single user defined <code>String</code>
	 * representation of the document we wish to parse. It then initializes the
	 * necessary data structures and starts the parsing process.
	 * 
	 * @param docBody
	 *            represents the document we are going to parse
	 * 
	 */
	...
	/**
	 * This method begins the parsing process. It's basic function is to
	 * separate document body text from tags.
	 * 
	 * @param text
	 *            the <code>String</code> representation of the document we are
	 *            parsing
	 * @throws SmartScriptParserException
	 *             if tags aren't properly closed
	 */
	private void parse(String text) {
		// this method's main job is to separate tags from normal text body by
		// searching for tag openers
		...
		while (index < text.length()) {
			...
			if (c == '{' && text.startsWith("$", (index + 1))) {
				...
				// we add the tag as a single element and shift around the
				// indices to skip over it
	...
	/**
	 * This method's main job is to begin tokenizing and analysing tags in our
	 * document. If tags are in valid format a document tree which represents
	 * our parsed document is created.
	 * 
	 * @throws SmartScriptParserException
	 *             if tags have invalid format
	 */
	private void tokenize() {
		...
		for (int i = 0, size = elements.size(); i < size; i++) {
			...
			if (isTagged(s)) {
				...
				// if the tag is defined start analyzing the format, call the
				// right method based on its name
				if (isValidTag(s)) {
	...
	/**
	 * This method is used for analyzing the format of a valid
	 * <code>{$=...$}</code> tag.
	 * <p>
	 * There is no predefined number of elements this tag can contain however
	 * only certain types are allowed and they all have their format.
	 * <p>
	 * - Variables must start with a letter followed by any number of digits
	 * letters or underscores <br>
	 * - Integers must only contain digits and no decimal point <br>
	 * - Doubles must contain only digits and a single decimal point <br>
	 * - Operators must be pre-defined, currently only supported operators are
	 * {+,-,*,/,%} <br>
	 * - Strings must be properly opened and closed \"example\" <br>
	 * - Functions must start with '@' followed by a valid function name
	 * 
	 * @param s
	 *            <code>String representation of a <code>{$=..$}</code> tag
	 * @throws SmartScriptParserException
		...
		while (s.length() != 0) {
			...
			if (Character.isLetter(c)) {
				...
				if (counterPoint == 0) {
					...
					// if a single decimal point was found the number is a
					// double
				} else if (counterPoint == 1) {
					double d = Double.parseDouble(s.substring(start, index));
					tokenarray.add(new TokenConstantDouble(d));
					// if more than one decimal point was found input was
					// invalid
				} else {
					throw new SmartScriptParserException(
							"invalid literal digit");
				}
				...
				tokenarray
						.add(new TokenOperator(s.substring(start, index + 1)));
				...
				if (c == '$') {
					tokenarray.add(new TokenString(s
				...
				if (Character.isLetter(c)) {
					...
					tokenarray
							.add(new TokenFunction(s.substring(start, index)));
	 ...
	 * <code>{$FOR ...$}</code> tag.
	 * <p>
	 * A valid for tag must start with a variable name followed by two or three
	 * parameters which can be digits variables or strings.
	 * <p>
	 * - Variables must start with a letter followed by any number of digits
	 * letters or underscores <br>
	 * - Integers must only contain digits and no decimal point <br>
	 * - Doubles must contain only digits and a single decimal point <br>
	 * - Strings must be properly opened and closed \"example\"
	 * 
	 * @param s
	 *            <code>String</code> representation of a for tag
	 * @throws SmartScriptParserException
	 *             if format is invalid
	 */
	private void forToken(String s) {
		...
		// initialize the node
		if (col.size() == 3) {
			forloop = new ForLoopNode((TokenVariable) col.get(0),
					(Token) col.get(1), (Token) col.get(2), null);
		} else {
			forloop = new ForLoopNode((TokenVariable) col.get(0),
					(Token) col.get(1), (Token) col.get(2), (Token) col.get(3));
		}
	 ...
	 * <p>
	 * Their sole purpose is to signify the end of non empty tags.
	 * 
	 * @param s
	 *            <code>String</code> representation of an END tag
	 */
	 ...
	 * @param index
	 *            the index where we have found the tag opener
	...
	/**
	 * This method checks if a given character represents a valid defined
	 * operator.
	 * 
	 * @param s
	 *            the operator we are testing
